 ...
 */
  ...
  async performOp(id:number) : Promise<Array<number>> {
    ...
    const flip_fns: Array<Promise<Draft>> = [];
    ...
    .then(flipped_drafts => {
      ...
      let outputs = op.perform(inputs);
      console.log("tree perform call 1: ", outputs);
      return outputs;
    })
    .then(res => { // unflip the drafts
      console.log("tree perform call 2: ", res);
      const flips = utilInstance.getFlips(this.ws.selected_origin_option, 3);
      let flipped = Promise.all(res.map(el => {
        console.log("mapped flip input: ", el);
        return flipDraft(el,  flips.horiz, flips.vert);
      }));
      console.log("flipped 1: ", flipped);
      console.log(res);
      return flipped; 
    })
    .then(flipped => { // mark this op node as complete
      opnode.dirty = false;
      console.log("flipped 2: ", flipped);
